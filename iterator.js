...
module.exports = class ModuleIterator {
   ...
   */
  *[Symbol.iterator]() {
    this._pipe = new ReadStream(this._wasm);
    ...
    while (!this._pipe.end) {
      ...
      const size = Number(unsigned.read(this._pipe));
...
class Section {
  constructor(sectionType, section, it, index) {
    ...
    const pipe = new ReadStream(section);
    if (this.type !== 'custom') {
      this.count = Number(unsigned.read(pipe));
    }
   ...
   */
  toJSON() {
    return wasm2json.sectionParsers[this.type](new ReadStream(this._section));
  }
  ...
  appendEntries(entries) {
    ...
    const bodyAndCount = Buffer.concat([unsigned.encode(this.count), this._body]);

    // encode length has save modifed section
    this._it._update(this._index, Buffer.concat([Buffer.from([this._type]), unsigned.encode(bodyAndCount.length), bodyAndCount]));
  }
}
